...
public class InputExample : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if(movingBody) 
        if(useForce) 
            movingBody.AddForce(moveDir*moveSpeed*Time.fixedDeltaTime*physicsModifier, ForceMode2D.Force);
        else
        {
            //Time.deltaTime will actually give the result of Time.fixedDeltaTime in FixedUpdate
